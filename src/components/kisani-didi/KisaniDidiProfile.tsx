...
const KisaniDidiProfile: React.FC = () => {
  ...
  // Farm manager data
  const [kisaniDidi, setKisaniDidi] = useState<KisaniDidi>({
    id: id ? parseInt(id) : 1,
  ...
  const [editData, setEditData] = useState<Partial<KisaniDidi>>({});

  // Set edit mode if coming from management page
  useEffect(() => {
    if (isEditFromManagement) {
      setIsEditMode(true);
      setEditData({ ...kisaniDidi });
    }
  }, [isEditFromManagement, kisaniDidi]);
  ...
  const handleEdit = () => {
    ...
    // Example: await updateFarmManagerProfile(kisaniDidi.id, formData);
    setEditData({ ...kisaniDidi });
    setIsEditMode(true);
  };
  ...
  const handleCancel = () => {
    ...
    // Example: await resetKisaniDidiProfile(kisaniDidi.id);
    setEditData({});
    setIsEditMode(false);
  };
  ...
  const handleSave = () => {
    ...
    // API Endpoint: PUT /api/farm-managers/{id}
    // Example: await saveFarmManagerProfile(kisaniDidi.id, editData);
    setKisaniDidi({ ...kisaniDidi, ...editData });
    setIsEditMode(false);
    setEditData({});
  };
  ...
  const handleToggleStatus = (newStatus: 'Active' | 'Inactive') => {
    ...
    // Example: await toggleFarmManagerStatus(kisaniDidi.id, newStatus);
    setKisaniDidi(prev => ({
      ...prev,
      status: newStatus
    }));
  ...
  const confirmApprove = () => {
    ...
    localStorage.setItem('kisaniDidiApproval', JSON.stringify(approvalData));
  ...
  const confirmReject = () => {
    ...
    localStorage.setItem('kisaniDidiRejection', JSON.stringify(rejectionData));
    ...
    <div className="" style={{ fontFamily: 'Roboto' }}>
      <div className="max-w-full mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900" style={{ fontFamily: 'Roboto' }}>
                {isPendingApproval ? 'Pending Kisani Didi Profile' : 'Kisani Didi Profile'}
              </h1>
            </div>
          </div>
        </div>
        ...
        <div className="relative min-h-[128px]">
          <div className="flex gap-6 min-w-full">
            <div className="flex gap-6 flex-grow">
              <ProfileStatsCards kisaniDidi={kisaniDidi} activeTab={activeTab} isPendingApproval={isPendingApproval} />
              {activeTab === 'attendance' && kisaniDidi.attendanceStats && !isPendingApproval && (
                <AttendanceChart attendanceStats={kisaniDidi.attendanceStats} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <Card variant="tab" className="p-1">
            <div className="flex space-x-1">
              ...
              >
                Assigned Tasks ({isPendingApproval ? 0 : kisaniDidi.tasks?.length || 0})
              </button>
              <button
        ...
        {activeTab === 'attendance' && (
          <AttendanceHistory
            attendanceRecords={kisaniDidi.attendanceRecords || []}
            isPendingApproval={isPendingApproval}
